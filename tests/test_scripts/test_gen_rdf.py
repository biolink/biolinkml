...
class GenRDFTestCase(ClickTestCase):
    ...
    def test_meta(self):
        ...
        self.maxDiff = None
        self.do_test(meta_yaml + f" --context {meta_context_path}", 'meta.ttl', filtr=filtr,
                     comparator=ClickTestCase.rdf_comparator)
        self.do_test(meta_yaml + f" --context {meta_contextn_path} --metauris", 'metan.ttl', filtr=filtr,
                     comparator=ClickTestCase.rdf_comparator)
        self.do_test(meta_yaml + f' -f n3  --context {meta_context_path}', 'meta.n3', filtr=filtr,
                     comparator=ClickTestCase.rdf_comparator)
        self.do_test(meta_yaml + f' -f xsv  --context {meta_context_path}', 'meta_error',
                     expected_error=click.exceptions.BadParameter)

    def test_make_script(self):
        """ Test a relative file path in JSON """
        self.do_test(meta_yaml + f" --context {source_context_path}",
