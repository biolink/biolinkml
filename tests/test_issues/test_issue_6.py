...
class DefinedPrefixTestCase(unittest.TestCase):
    def test_dct_prefix(self):
        ...
        with self.assertRaises(ValueError, msg="A colon in an identifier is illegal"):
            shex = loads(ShExGenerator(os.path.join(sourcedir, 'Issue_6.yaml'), fmt='json').serialize())
        shex = loads(ShExGenerator(os.path.join(sourcedir, 'Issue_6_fixed.yaml'), fmt='json').serialize())
